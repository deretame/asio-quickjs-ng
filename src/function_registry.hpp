...
using AsyncFunction = std::function<
    async_simple::coro::Lazy<qjs::Value>(qjs::Ctx, const std::vector<qjs::Value> &)>;
...
qjs::Value to_value(JSContext *ctx, R &&r) {
  ...
  if constexpr (std::is_void_v<T>) {
    ...
    std::string_view s = r;
    return qjs::Value::take(ctx,
                            JS_NewStringLen(ctx, s.data(), s.size()));
...
template <typename T>
T pull_arg_at(JSContext *ctx, const std::vector<qjs::Value> &args, std::size_t i) {
  if (i >= args.size()) {
...
qjs::Value invoke_sync_impl(JSContext *ctx, Fn &fn,
  ...
  auto call_args = std::tuple{
      pull_arg_at<std::tuple_element_t<I, Tuple>>(ctx, args, I)...};
...
invoke_async_impl(JSContext *ctx, Fn &fn,
                  const std::vector<qjs::Value> &args,
                  std::index_sequence<I...>) {
  if (args.size() < std::tuple_size_v<Tuple>) {
    JS_ThrowTypeError(ctx, "not enough arguments");
...
auto make_async_wrapper(Fn &&fn) {
  ...
  return [fn = std::forward<Fn>(fn)](
             qjs::Ctx ctx,
             const std::vector<qjs::Value> &args) -> async_simple::coro::Lazy<qjs::Value> {
